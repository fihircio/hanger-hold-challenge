...
const GameScreen: React.FC<GameScreenProps> = ({ isHolding, onHoldStart, onHoldEnd }) => {
  ...
  const handleGameEnd = async () => {
    ...
    // Pass measured finalTime to parent so it doesn't rely on ref timing
    // Prefer letting App compute/validate duration from its own startTimeRef by calling onHoldEnd()
    // without a measured value when sensor-based triggers are used. However, when button
    // release calls this function we provide measured `finalTime` as a fallback.
    onHoldEnd(finalTime);
  ...
  useEffect(() => {
    ...
    if (window.electronAPI) {
      ...
      // Initialize sensor service with enhanced error handling
      arduinoSensorService.initialize().then(() => {
        ...
        arduinoSensorService.setEventHandlers({
          ...
          onSensorEnd: (ts?: number) => {
            ...
            const WAIT_MS = 50;
            setTimeout(() => {
              // Pass end timestamp and mark it as a timestamp via second parameter
        ...
        // Enable sensor with error handling
        try {
  ...
  const handleResetCounts = () => {
    ...
    console.log('[GameScreen] Slot counts reset requested');
