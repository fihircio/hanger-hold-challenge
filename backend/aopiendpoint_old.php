    ...
    elseif ($path === '/api/inventory/stats' || $path === '/api/inventory/stats/') {
        ...
        echo json_encode([
            ...
            'data' => [
                ...
                'bronze_usage_percentage' => $bronzeSlots > 0 ? round(($bronzeDispensed / ($bronzeSlots * 5)) * 100, 1) : 0
            ]
        ]);
    }
...
function handlePostRequest($conn, $path) {
    ...
    if ($path === '/api/electron-vending/log' || $path === '/api/electron-vending/log/') {
        ...
        foreach ($requiredFields as $field) {
            // Allow empty strings and 0 values for optional fields, only check if key exists
    ...
    // API Scores endpoint - Create new score (for frontend compatibility)
    elseif ($path === '/api/scores' || $path === '/api/scores/') {
    ...
    // Spring SDK dispensing endpoint (Updated for 3-tier system)
